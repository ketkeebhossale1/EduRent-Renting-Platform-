...
export function ObjectDetailModal({ object, isOpen, onClose, currentUser, onBookingComplete }: ObjectDetailModalProps) {
  ...
  const handleBooking = () => {
    ...
    // Save transaction
    const transactions = JSON.parse(localStorage.getItem(`rentify_transactions_${currentUser}`) || "[]");
    transactions.unshift(transaction);
    localStorage.setItem(`rentify_transactions_${currentUser}`, JSON.stringify(transactions));

    // If deposit is included, save deposit record
    if (includeDeposit) {
      ...
      const deposits = JSON.parse(localStorage.getItem(`rentify_deposits_${currentUser}`) || "[]");
      deposits.unshift(depositRecord);
      localStorage.setItem(`rentify_deposits_${currentUser}`, JSON.stringify(deposits));
    }
